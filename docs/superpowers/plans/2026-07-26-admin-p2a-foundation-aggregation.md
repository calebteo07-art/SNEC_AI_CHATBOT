...
- **pytest 9.0.3 aborts the whole session on a collection error** (`Interrupted: 1 error during collection`). Every Step 2 in this plan that expects a `ModuleNotFoundError` from a not-yet-created module therefore needs **`--continue-on-collection-errors`** to show the other files' results; without it you see only the interrupt. Related: a test written as `from tools.supervisor import x` fails with `ImportError: cannot import name 'x'`, not `ModuleNotFoundError` — same root cause, different text, so don't treat the wording mismatch as a problem.
...
> **AMENDMENT — post-review, this is what shipped.** The Step 1 block above is the
> **delivered** test file, byte for byte, not the text originally drafted. Three things
> changed during Task 11, and Task 12 should read the block as fact rather than intent:
>
> - **`unknown_tag_attempts` is rendered**, via `driftNote()` in the view-model, gated on
>   `sources.flashcard`. Task 9's amendment required this of Task 11 and Task 11's body
>   never carried it — the same propagation failure that amendment itself names. Section 8
>   of the test pins the gate.
> - **The drafted test could not fail for one of its own rules.** `scoreReadout`'s null
>   branch was unreachable from every drafted fixture (the one `NO_DATA` assertion takes
>   `comparisonPanel`'s `f ? … : NO_DATA` ternary and never calls it), so a `scoreReadout`
>   returning `"0% (0)"` passed the whole suite. Found by mutation testing, not by reading.
>   The flashcards-only group now reaches it.
> - **The `g()` default paired `weakness_score: null` with `low_confidence: false`**, a row
>   the endpoint cannot emit: `cohort_analytics.py:378` always returns `low_confidence: True`
>   alongside a null score, and `:271-299` makes a signal present only when its metric is
>   non-null AND its own denominator positive — so a fixture carrying `safety_gradable_n` or
>   `missed_top` has a safety signal and therefore a score. The default is now `true` and
>   every scored fixture states its own confidence explicitly. Flipping the default without
>   repairing the call sites fails the first ranking assertion, so the repair is load-bearing,
>   and all 21 mutations still die afterwards — no assertion went vacuous.
>
> The general lesson, now three for three in this plan: **a fixture that encodes a state the
> wire cannot produce pins nothing, and reads as coverage.** Check new fixtures against the
> producer, not against the other fixtures.
