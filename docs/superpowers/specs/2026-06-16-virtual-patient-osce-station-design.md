...
**Non-goals (YAGNI)**
- Not editing the ~150 case JSON files to add demographics or checklist links (handled by
  the resolver + patient prompt instead).
...
The identity step needs name + NRIC/DOB/address. Rather than edit ~150 case files, extend
`PATIENT_SYSTEM` so the patient, when asked, gives **realistic, internally-consistent
Singapore demographics** (NRIC format `S#######A`, a plausible address, DOB consistent with
`patient.age`) and stays consistent for the rest of the consult. No schema change.
...
- Demographics generated by the patient prompt (no case-file edits).
