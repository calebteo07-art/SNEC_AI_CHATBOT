...
/* React-context replacement for react-router's outlet context: AdminLayout
 * provides it, the admin tab pages consume it. */
const AdminCtx = createContext<AdminOutletContext | null>(null);
export const AdminCtxProvider = AdminCtx.Provider;
export function useAdminOutlet(): AdminOutletContext {
  const ctx = useContext(AdminCtx);
  if (!ctx) throw new Error("useAdminOutlet must be used inside AdminLayout");
  return ctx;
}
