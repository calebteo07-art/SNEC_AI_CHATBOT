...
// The /admin block that used to live here moved to frontend/tests/console_assert.mjs.
// It asserted against the .aurora-admin dark surface, which no longer exists — /admin is
// now a full-bleed console outside the student shell, with its own harness. Everything
// that mattered went with it: the AdminGuard checks (trainer admitted, student bounced),
// the a11y landmark contract, at-risk KPI-to-list agreement, the discipline re-query, the
// D13 null-bucket gap, failed-read-renders-as-error, the note-draft poll regression and
// the mastery scales. Assertions tied to panels the rebuild deliberately cut went with
// those panels. This file drives the STUDENT app; do not re-add /admin coverage here.
