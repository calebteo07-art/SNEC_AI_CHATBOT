...
- [ ] 014_audit_events.sql — **PENDING APPLICATION** (durable `audit_events` table for the admin privilege-lifecycle trail: approve/create-staff/promote/demote/unapprove, previously unattributed. `db.insert_audit_event` is best-effort and swallows the missing-table error, so admin actions keep working and simply record nothing until applied)
