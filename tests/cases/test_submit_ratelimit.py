...
def _stub_profile_read():
    ...
    The test pins every other db call but not this one, so it read production Supabase —
    once per submit, 11 times over the rate-limit loop — and a miss makes
    `tools.profile.get_profile` CREATE the row (db.upsert_profile), so it wrote too. The
    subject is the 429, which this doesn't touch. See `_forbid_real_supabase` in
    tests/conftest.py.
    """
